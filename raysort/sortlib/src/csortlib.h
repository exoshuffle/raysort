...
namespace csortlib
{
  ...
  std::vector<Partition> SortAndPartition(const Array<Record> &record_array,
                                          const std::vector<Key> &boundaries, bool skip_sorting = false);
  ...
  {
  public:
    FileMerger(const std::vector<std::string> &input_files, const std::string &output_file,
               size_t input_batch_bytes, size_t output_batch_records);
