...
class ExternalStorageSmartOpenImpl(ExternalStorage):
    ...
    def __init__(
        ...
        if self.is_for_s3:
            ...
            self.transport_params = {"defer_seek": True, "resource": self.s3, "buffer_size": 1024 * 1024}
        else:
    ...
    def spill_objects(self, object_refs, owner_addresses) -> List[str]:
        ...
        url = f"{uri}/{key}"
        with open(url, mode="wb", buffering=1024*1024, transport_params=self.transport_params) as file_like:
