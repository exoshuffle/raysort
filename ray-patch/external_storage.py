...
class ExternalStorageRayStorageImplArrowBoto(ExternalStorageRayStorageImplArrow):
    def restore_spilled_objects(
        self, object_refs: List[ObjectRef], url_with_offset_list: List[str]
    ):
        total = 0
        for i in range(len(object_refs)):
            ...
            bucket, object_key = url.split("/", maxsplit=1)
            end = offset + size - 1
            ...
            body = resp["Body"]
            f = io.BufferedReader(body._raw_stream, buffer_size=self._buffer_size)
            # Read a part of the file and recover the object.
            address_len = int.from_bytes(f.read(8), byteorder="little")
            metadata_len = int.from_bytes(f.read(8), byteorder="little")
            buf_len = int.from_bytes(f.read(8), byteorder="little")
